...
const SignUP = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (fname && lname && email && password && role) {
      ...
      if (response) {
        ...
        setLoading(false);
        toast.success("There was a Server side Problem");
      }
    } else {
  ...
  return (
    <div className="w-screen min-h-[80vh] h-fit flex justify-center items-center bg-gray-100">
      <div className="bg-white p-6 rounded-lg shadow-md w-80 text-center">
        ...
        <span className="flex items-center text-gray-500 gap-1 pt-2 justify-center">
          Already have an Account ?{" "}
          <Link href="signin" className="text-sm hover:underline text-sky-500">
            sign In
          </Link>
        </span>
      </div>
